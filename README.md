...
Identifying cancer types and subtypes from molecular data is a foundational problem in computational biology. In 1999, Golub et al. published a landmark study in *Science* demonstrating that machine learning applied to gene expression microarray data could accurately classify acute lymphoblastic leukemia (ALL) versus acute myeloid leukemia (AML) with 85.3% accuracy on an independent test set. This work, now cited over 20,000 times, established the paradigm of expression-based cancer diagnostics. In this study, we reproduce Golub's classification pipeline using modern Python/scikit-learn tools on the original leukemia dataset (72 samples, 7,129 genes), then test whether the same methodology generalizes to a substantially harder problem: classifying six molecular subtypes of breast cancer from an independent microarray dataset (GSE45827, 151 samples, 54,675 genes). We implement Golub's signal-to-noise ratio (S2N) for feature selection and compare it with ANOVA F-test. Eight classifiers are evaluated, including Nearest Centroid (Golub's analog), Linear SVM, RBF SVM, Random Forest, KNN, Logistic Regression, and Gradient Boosting. Our Nearest Centroid classifier achieved 82.4% on the leukemia test set, closely reproducing Golub's 85.3%. Modern classifiers outperformed the 1999 approach: Linear SVM reached 91.2%, and grid-search SVM achieved 94.1%. On breast cancer, the same pipeline achieved 98.7% accuracy with optimized SVM. Linear SVM achieved 96.0% CV accuracy on both datasets, demonstrating that the gene expression-based ML classification paradigm is both reproducible and generalizable across cancer types, microarray platforms, and classification complexity.
